...
"<title>Smart Home IoT - Device Config</title>"
"<style>"
...
".icon{width:56px;height:56px;background:linear-gradient(135deg,#6366f1,#8b5cf6);border-radius:14px;display:inline-flex;align-items:center;justify-content:center;font-size:28px;margin-bottom:12px;box-shadow:0 8px 20px rgba(99,102,241,0.4)}"
"h1{font-size:22px;font-weight:700;letter-spacing:-0.5px;color:#fff}"
".badge{display:inline-block;background:rgba(99,102,241,0.15);color:#818cf8;border:1px solid rgba(99,102,241,0.3);font-size:12px;padding:4px 10px;border-radius:20px;margin-top:6px;font-weight:500}"
...
"input:focus,select:focus{border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,0.25)}"
...
".toggle-btn:focus{outline:none;color:#818cf8}"
...
".btn-primary{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;box-shadow:0 4px 14px rgba(99,102,241,0.4);margin-top:22px}"
...
".scan-btn{background:transparent;border:1px dashed #6366f1;color:#818cf8;padding:8px;font-size:12px;border-radius:8px;cursor:pointer;width:100%;margin-bottom:8px;font-weight:500}"
".scan-btn:hover{background:rgba(99,102,241,0.1)}"
".footer{text-align:center;margin-top:20px;font-size:11px;color:#64748b}"
"</style></head><body><div class='card'>"
"<div class='header'>"
"<div class='icon'>⚙️</div>"
"<h1>Device Configuration</h1>"
"<span class='badge'>Node: Temperature &amp; Humidity</span>"
"</div>"
...
static void handleRoot() {
  ...
  html += "<input type='text' id='pairing_code' name='pairing_code' value='" + String(config.pairingCode) + "' placeholder='e.g. TH-7788' required maxlength='64'>";
  html += "</div>";
  ...
  html += "<div class='footer'>Hardware MAC: " + mac + "<br>Config Portal Active &bull; Normal Loop Paused</div>";
...
static void handleSave() {
  ...
  "<p>The ESP node is rebooting now and connecting with new credentials...</p>"
...
void startConfigPortal() {
  ...
  cleanMac.replace(":", "");
  String apName = String("SmartHome-Node-") + cleanMac.substring(cleanMac.length() > 4 ? cleanMac.length() - 4 : 0);
  ...
  // Pauses all normal telemetry & sensor functions
  while (portalRunning) {
