...
static const char* stateTopic = "iot/state";
...
// Callback for incoming MQTT commands (open, close, stop, set_position, restart, ota, etc.)
static void mqttCallback(char* topic, byte* payloadBytes, unsigned int length) {
  ...
  // Smart Curtain Specific Actions
  if (strcmp(action, "open") == 0) {
    Serial.println("[MQTT] Action: OPEN curtain");
    openCurtain();
  } else if (strcmp(action, "close") == 0) {
    Serial.println("[MQTT] Action: CLOSE curtain");
    closeCurtain();
  } else if (strcmp(action, "stop") == 0) {
    Serial.println("[MQTT] Action: STOP curtain");
    stopCurtain();
  } else if (strcmp(action, "set_position") == 0) {
    int targetPos = doc["position"] | getCurtainPosition();
    Serial.printf("[MQTT] Action: SET_POSITION -> %d%%\n", targetPos);
    setCurtainPosition(targetPos);
  }
  // Standard Base Infrastructure Actions
  else if (strcmp(action, "restart") == 0 || strcmp(action, "reboot") == 0) {
...
void maintainMQTT() {
  // If WiFi is disconnected, attempt WiFi reconnect periodically without blocking loop execution
  if (!isWiFiConnected()) {
    unsigned long currentMillis = millis();
    if (currentMillis - lastWifiRetry >= RECONNECT_INTERVAL_MS) {
  ...
  // If WiFi is connected but MQTT is not, attempt non-blocking MQTT connect periodically
  if (!mqttClient.connected()) {
    unsigned long currentMillis = millis();
    if (currentMillis - lastMqttRetry >= RECONNECT_INTERVAL_MS) {
      ...
      cleanMac.replace(":", "");
      String clientId = String("ESP-Curtain-") + cleanMac.substring(cleanMac.length() > 6 ? cleanMac.length() - 6 : 0);
...
bool publishCurtainState(int position, const char* state) {
  ...
  // Construct authenticated JSON payload (SmartCurtainStateDto)
  JsonDocument doc;
  ...
  if (mqttClient.publish(stateTopic, jsonBuffer)) {
    Serial.print("[MQTT] Curtain state published -> ");
    Serial.print(stateTopic);
    Serial.print(" : ");
    Serial.println(jsonBuffer);
    return true;
  } else {
    Serial.println("[MQTT] Error: Failed to publish curtain state message!");
    return false;
  }
}
