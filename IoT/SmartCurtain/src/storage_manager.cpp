...
static void loadDefaults() {
  ...
  currentConfig.lastPosition = 0; // Default closed (0%)
}
...
void initStorage() {
  ...
  if (strncmp(currentConfig.magic, CONFIG_MAGIC, 4) != 0) {
    ...
    Serial.print("          Saved Position  : "); Serial.print(currentConfig.lastPosition); Serial.println("%");
  }
}
...
bool saveCurtainPosition(uint8_t position) {
  ...
  if (currentConfig.lastPosition == position) return true;
  
  currentConfig.lastPosition = position;
  EEPROM.put(0, currentConfig);
  bool success = EEPROM.commit();
  if (success) {
    Serial.printf("[Storage] Curtain position updated to %d%% in EEPROM.\n", position);
  }
  return success;
}

bool saveConfig(const AppConfig& newConfig) {
  uint8_t prevPos = currentConfig.lastPosition;
  currentConfig = newConfig;
  currentConfig.lastPosition = prevPos;
  memcpy(currentConfig.magic, CONFIG_MAGIC, sizeof(currentConfig.magic));
  EEPROM.put(0, currentConfig);
  bool success = EEPROM.commit();
  if (success) {
